    ...
    <body>
		...
		<section>
			<p>
				...
				<?php 
					$req5 = $bdd->prepare("SELECT E.EMPR_RETOUR_THEORIQUE, F.FILM_TITRE FROM EMPRUNT AS E INNER JOIN SUPPORT AS S ON E.SUP_ID = S.SUP_ID INNER JOIN FILM AS F ON S.FILM_ID = F.FILM_ID WHERE UTI_ID = ? AND EMPR_RENDU = FALSE");
					...
					while($donnees5 = $req5->fetch()){
						echo htmlspecialchars($donnees5['FILM_TITRE']), "   à rendre le  ", date("d/m/Y", strtotime($donnees5['EMPR_RETOUR_THEORIQUE'])), "     ";
